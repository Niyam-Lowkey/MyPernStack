...
export const getProducts = catchAsync(async (req: Request, res: Response, next: NextFunction) => {
  ...
  if (puffCountMin) {
    queryParams.push(parseInt(puffCountMin as string, 10));
    whereClauses.push(`p.puff_count >= $${queryParams.length}`);
  }
  if (puffCountMax) {
    queryParams.push(parseInt(puffCountMax as string, 10));
    whereClauses.push(`p.puff_count <= $${queryParams.length}`);
  }

  // Filter by nicotine strength
  if (nicotineStrength) {
    queryParams.push(parseFloat(nicotineStrength as string));
  ...
  const safeSort = allowedSortFields.includes(sort as string) ? sort : 'created_at';
  ...
  const productsResult = await query(fetchSql, fetchParams);
...
export const getProductByIdOrSlug = catchAsync(async (req: Request, res: Response, next: NextFunction) => {
  ...
  const productResult = await query(prodSql, [idOrSlug]);
  ...
  const flavorsResult = await query(
    "SELECT id, flavor_name, status FROM flavors WHERE product_id = $1 AND status = 'active' ORDER BY flavor_name ASC",
    [product.id]
  );
...
export const createProduct = catchAsync(async (req: Request, res: Response, next: NextFunction) => {
  ...
  if (slugCheck.rowCount && slugCheck.rowCount > 0) {
    return next(new AppError('A product with a similar name already exists', 400));
  }
  ...
  if (flavors && Array.isArray(flavors)) {
    for (const fName of flavors) {
      if (typeof fName === 'string' && fName.trim().length > 0) {
        const flavRes = await query(
          'INSERT INTO flavors (product_id, flavor_name, status) VALUES ($1, $2, $3) RETURNING *',
          [product.id, fName.trim(), 'active']
        );
...
export const updateProduct = catchAsync(async (req: Request, res: Response, next: NextFunction) => {
  ...
  if (image && image !== product.image) {
    const imgCheck = await query('SELECT id FROM product_images WHERE product_id = $1 AND sort_order = 0', [id]);
    if (imgCheck.rowCount && imgCheck.rowCount > 0) {
      await query('UPDATE product_images SET image_url = $1 WHERE product_id = $2 AND sort_order = 0', [image, id]);
    } else {
      await query('INSERT INTO product_images (product_id, image_url, sort_order) VALUES ($1, $2, 0)', [id, image]);
    }
  }
